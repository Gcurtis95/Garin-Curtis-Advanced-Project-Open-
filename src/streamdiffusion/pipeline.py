...
class StreamDiffusion:
    ...
    def prepare(
        ...
        delta: float = 1.0,
        generator: Optional[torch.Generator] = None,  # Pass generator explicitly
        seed: Optional[int] = None,
    ) -> None:
        if generator is None:
            self.generator = torch.Generator(device=self.device).manual_seed(seed or 42)  # Fallback seed
        else:
