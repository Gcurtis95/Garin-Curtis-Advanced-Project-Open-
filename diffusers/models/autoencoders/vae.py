...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        ## potential to add transformation here
    ...
    def forward(
        ...
        # sample = dialation(sample)
        
        # sample = erosion(sample)
        # sample = sobel(sample)
        
        # sample = gradeint(sample)
        
        # sample = add_rand_cols(sample)
        
        # sample = inversion(sample)
        
        # sample = add_normal(sample)
        
        
        # sample = rotate_y(sample, 1.2)
        
        
        # sample = rotate_z(sample, 1.2)
...
def rotate_X(x, angle=1.6):
    
    
    
    # Convert angle to a torch tensor
    angle_tensor = torch.tensor(angle, dtype=torch.float16)
    
    # Rotation matrix for 2D rotation around the X-axis
    ...
    device = x.device
    op = torch.tensor(rotation_matrix, dtype=torch.float16).to(device)
    
    # Apply the rotation to each channel independently
    rotated_x = torch.zeros_like(x)
    for i in range(x.size(0)):  # Iterate over channels
        for j in range(x.size(1)):
            channel = x[i, j]
            channel_2d = channel.view(2, -1)  # Reshape to 2D for rotation
            rotated_channel_2d = torch.tensordot(op, channel_2d, dims=1)
            rotated_channel = rotated_channel_2d.view_as(channel)  # Reshape back to original
            rotated_x[i, j] = rotated_channel
...
# original rotate transforms

# def rotate(x, angle):
#     # Ensure the input tensor is in float32
...
#     device = x.device
#     op = torch.tensor(rotation_matrix, dtype=torch.float32).to(device)
...
#     # Standard deviation
#     sigma = 1  # You can adjust this value as desired

#     # Generate a 2D Gaussian distribution with peak at tse center and specified standard deviation
...
#     kernel = torch.ones(r, r, dtype=torch.float16).to(x.get_device())
...
#     kernel = torch.ones(4, 4, dtype=torch.float16).to(x.get_device())
